...
//
// The package also normalizes the values that end up as graph nodes: names
// ([Entity]), dates ([Date]), numbers ([Number]), quantities ([Measure]) and
// tables of records ([Rows]). Those return typed values and an error rather
// than a string, because a date that failed to parse is not a date.
package normalize
